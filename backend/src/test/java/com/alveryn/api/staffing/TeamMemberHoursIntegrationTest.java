...
class TeamMemberHoursIntegrationTest {
  ...
  @Test void reportsServerCalculatedNetHoursAndRejectsUnprivilegedOrInvalidReads() throws Exception {
    ...
    mvc.perform(put("/api/organizations/{org}/staffing/members/{member}/days/{date}",org,member,"2026-08-31").header(HttpHeaders.AUTHORIZATION,token(owner)).contentType(MediaType.APPLICATION_JSON).content("{\"type\":\"VACATION\",\"notes\":\"Approved leave\"}")).andExpect(status().isOk());
    mvc.perform(get("/api/organizations/{org}/staffing/members/{member}/hours",org,member).param("from","2026-08-30").param("to","2026-08-31").header(HttpHeaders.AUTHORIZATION,token(owner)))
        .andExpect(status().isOk()).andExpect(jsonPath("$.data.plannedMinutes").value(210)).andExpect(jsonPath("$.data.workedMinutes").value(210)).andExpect(jsonPath("$.data.days[0].correctedSessions").value(1)).andExpect(jsonPath("$.data.days[0].openSessions").value(0)).andExpect(jsonPath("$.data.weeks[0].workedMinutes").value(210)).andExpect(jsonPath("$.data.months[0].workedMinutes").value(210)).andExpect(jsonPath("$.data.sessions[0].unitName").value("Berlin")).andExpect(jsonPath("$.data.sessions[0].workTypeName").value("Night")).andExpect(jsonPath("$.data.sessions[0].state").value("CORRECTED")).andExpect(jsonPath("$.data.absences[0].type").value("VACATION"));
    ...
    mvc.perform(get("/api/organizations/{org}/staffing/members/{member}/hours",org,member).param("from","2026-08-30").param("to","2026-08-31").param("limit","101").header(HttpHeaders.AUTHORIZATION,token(owner))).andExpect(status().isBadRequest());
