  ...
  static final class AuthPropertiesValidator {
    AuthPropertiesValidator(AuthProperties properties, Environment environment) {
      ...
      if (secret == null || secret.isBlank()) {
        throw new IllegalStateException("roomly.auth.jwt-secret must not be blank");
      }
      if (secret.length() < 32) {
        throw new IllegalStateException("roomly.auth.jwt-secret must be at least 32 characters long");
      }
