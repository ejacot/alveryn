...
For local development, the backend defaults to `jdbc:postgresql://localhost:5432/roomly` with username `roomly` and password `change-me`, so it can start directly from IntelliJ IDEA once PostgreSQL is running. Environment variables still override these defaults when needed.

The backend uses Java 21, PostgreSQL, Flyway, Hibernate, and the Java package `com.roomly.api`.

## Configuration

The backend accepts `DB_URL`, `DB_USERNAME`, and `DB_PASSWORD`. `DB_URL` must be a JDBC URL such as `jdbc:postgresql://host:5432/roomly`. Local defaults are provided for development only; deployment environments should supply their own values.
