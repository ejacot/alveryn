...
- `work_types`: user-owned work formulas; can be simple or parent/child through `parent_work_type_id`.
- `work_records`: one job or multi-day project for a user, with start/end dates, optional address, team size and notes.
- `work_record_lines`: performed work lines inside a record, each pointing to a `work_type` and storing calculation snapshots.
