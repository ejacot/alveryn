...
Roomly uses feature-owned application services as transactional boundaries. Until authentication exists, every ownership-sensitive operation receives `userId` explicitly. Repositories contain persistence queries only; cross-entity and conflict decisions belong to services.
