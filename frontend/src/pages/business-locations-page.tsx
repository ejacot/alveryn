...
export function BusinessLocationsPage() {
  ...
  return <BusinessManagementShell><div className="business-admin">
    <header className="business-admin__header"><div><p>{t("management.locations.eyebrow")}</p><h1>{t("teams.title")}</h1><span>{t("management.locations.description")}</span></div><button onClick={beginCreate}><Plus />{t("teams.add")}</button></header>
    <div className="business-admin__layout"><section className="business-admin__panel"><div className="business-admin__panel-title"><MapPinned /><div><h2>{t("teams.title")}</h2><p>{t("management.locations.count", { count: (units.data ?? []).length })}</p></div></div><div className="business-admin__list">{(units.data ?? []).map((unit) => <button key={unit.id} type="button" className={`business-admin__row business-admin__row-button ${selectedId === unit.id ? "is-selected" : ""}`} style={{ paddingLeft: unit.parentId ? 42 : 18 }} onClick={() => setSelectedId(unit.id)}><span className="business-admin__avatar"><MapPinned /></span><span><strong>{unit.name}</strong><small>{t(`unitTypes.${unit.type}`)} · {t(`checkIn.${unit.checkInMode}`)}</small></span><span className={`business-admin__status ${unit.active ? "is-active" : "is-suspended"}`}>{t(unit.active ? "management.locations.active" : "management.locations.inactive")}</span><Pencil className="business-admin__edit-icon" /></button>)}</div></section>
      <aside className="business-admin__panel business-admin__form"><div><p>{t(selected ? "management.locations.edit" : "management.locations.new")}</p><h2>{selected ? selected.name : t("teams.add")}</h2></div><Input label={t("teams.name")} value={name} onChange={(event) => setName(event.target.value)} /><Select label={t("teams.parent")} value={parentId} onChange={(event) => setParentId(event.target.value)}><option value="">{t("teams.noParent")}</option>{(units.data ?? []).filter((unit) => unit.id !== selectedId).map((unit) => <option key={unit.id} value={unit.id}>{unit.name}</option>)}</Select><Select label={t("teams.type")} value={type} onChange={(event) => setType(event.target.value as OrganizationUnit["type"])}>{(["LOCATION", "DEPARTMENT", "TEAM", "OTHER"] as const).map((value) => <option key={value} value={value}>{t(`unitTypes.${value}`)}</option>)}</Select><Select label={t("teams.checkIn")} value={checkInMode} onChange={(event) => setCheckInMode(event.target.value as OrganizationUnit["checkInMode"])}>{(["DISABLED", "OPTIONAL", "REQUIRED"] as const).map((value) => <option key={value} value={value}>{t(`checkIn.${value}`)}</option>)}</Select><button className="business-admin__primary" disabled={!name.trim() || save.isPending} onClick={() => save.mutate()}>{selected ? t("save") : t("teams.action")}</button>{selected ? <button className="business-admin__secondary" disabled={toggle.isPending} onClick={() => toggle.mutate()}><Power />{t(selected.active ? "management.locations.deactivate" : "management.locations.reactivate")}</button> : null}</aside>
