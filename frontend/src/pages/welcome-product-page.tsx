...
import { CalendarCardImage, DashboardCardImage, PublicFooter, PublicHeader, RegistrationLink, StatisticsCardImage, useStoredPublicTheme } from "./welcome-page";
  ...
  return (
    <main className="landing-page fixed inset-0 overflow-y-auto bg-black text-white">
      ...
      <section className="mx-auto max-w-7xl px-5 pb-12 pt-14 text-center sm:px-8 sm:pt-20">
        <Link to="/welcome" className="mb-8 inline-flex items-center text-sm font-semibold text-white/58 transition hover:text-white focus:outline-none focus:ring-2 focus:ring-emerald-400/40">
          <ArrowLeft className="mr-2 h-4 w-4" aria-hidden="true" />{t("productPages.back")}
        </Link>
        ...
        {cards.map((card) => (
          <article key={card} className={`overflow-hidden rounded-[30px] border border-white/[0.09] bg-white/[0.035] ${product === "dashboard" && card === "day" ? "md:col-span-2" : ""}`}>
            <div className={`grid gap-3 p-3 sm:p-4 ${product === "dashboard" && card === "day" ? "sm:grid-cols-[0.82fr_1.18fr] sm:items-start" : ""}`}>
              <ProductImage product={product} card={card} alt={t(`${namespace}.cards.${card}.title`)} />
              {product === "dashboard" && card === "day" ? <DashboardCardImage name="flow" alt={t("dashboardTour.cards.day.body")} /> : null}
              {product === "statistics" && card === "trend" ? <StatisticsCardImage name="kpis" alt={t("statisticsTour.cards.trend.body")} /> : null}
            </div>
      ...
      <section className="border-t border-white/[0.07] px-5 py-16 text-center sm:px-8">
        ...
        <RegistrationLink className="mt-7 inline-flex items-center rounded-full bg-white px-6 py-3 text-sm font-semibold text-black">
...
function ProductImage({ product, card, alt }: { product: WelcomeProduct; card: string; alt: string }) {
  if (product === "dashboard") return <DashboardCardImage name={card as "day" | "activity" | "rhythm"} alt={alt} />;
  if (product === "calendar") return <CalendarCardImage name={card as "month" | "summary" | "payroll" | "flow" | "rhythm" | "tools"} alt={alt} />;
  return <StatisticsCardImage name={card as "filters" | "trend" | "compare" | "selected"} alt={alt} />;
}
