...
export type TeamHoursAbsence = {date:string;type:"VACATION"|"SICK";notes:string|null};
export type TeamMemberHours = { membershipId:string;memberName:string;from:string;to:string;daysWorked:number;plannedMinutes:number;workedMinutes:number;days:TeamHoursDay[];weeks:TeamHoursPeriod[];months:TeamHoursPeriod[];sessions?:TeamHoursSession[];totalSessions?:number;offset?:number;absences?:TeamHoursAbsence[] };
...
export type PersonalStaffingAssignmentResult = Omit<StaffingAssignmentResult, "assignmentId" | "organizationId" | "organizationName" | "memberName" | "date" | "workTypeName" | "workTypeCode" | "unitName">;
export type PersonalBusinessAssignment = { id: string; date: string; unitId: string; unitName: string; workTypeId: string; workTypeCode: string; workTypeName: string; color: string; startTime: string | null; endTime: string | null; checkInMode: "DISABLED" | "OPTIONAL" | "REQUIRED"; result: PersonalStaffingAssignmentResult | null };
export type PersonalStaffingDayEntry = { id: string; date: string; type: "REST_DAY" | "VACATION" | "SICK"; notes: string | null; hasWorkConflict: boolean };
export type PersonalBusinessSchedule = { organizationId: string; organizationName: string; from: string; to: string; assignments: PersonalBusinessAssignment[]; dayEntries: PersonalStaffingDayEntry[] };
