...
export function StatisticsSkeleton() {
  return (
    <div className="space-y-5 pb-6" aria-hidden="true">
      ...
      <Card variant="section" className="h-32 animate-pulse" />
      <Card variant="section" className="h-52 animate-pulse" />
      <div className="grid grid-cols-2 gap-3">
        {Array.from({ length: 4 }).map((_, index) => (
          <Card key={index} variant="muted" className="h-24 animate-pulse" />
        ))}
      </div>
    </div>
  );
...
export function StatisticsEmptyState() {
  ...
  return (
    <Card as="section" variant="section" className="py-8 text-center">
...
export function StatisticsHeatmapPlaceholder() {
  const { t } = useTranslation("common");
  return (
    <Card as="section" variant="section" aria-labelledby="statistics-heatmap-title">
